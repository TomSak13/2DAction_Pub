...
public class MonsterDogManager : EnemyManager
{
    public const float DefaultDamage = 10f;
    [SerializeField] GameObject _deathEffect;

    private AIMonsterDogBrain _brain;

    private void Start()
    {
        _rigidbody2D = GetComponent<Rigidbody2D>();
        _direction = DirectionType.Right;
        _enemyType = EnemyType.MonsterDog;

        _brain = new AIMonsterDogBrain();
    ...
    private void Update()
    {
        ...
        _brain.UpdateSymbol(gameObject, transform.position);

        /* 選択するゴールの更新 */
        _brain.UpdateRoute(gameObject);
    }
    ...
    {
        /* AI_Sensorで各値を更新 */
        _brain.UpdateSensor(gameObject, transform.position);

        _brain.ExecuteAction(gameObject);
